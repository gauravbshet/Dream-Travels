...
export function PackageCard({
  ...
  return (
    <Link href={href} className={cn("group block premium-card-wrapper", className)}>
      <article className="h-full">
        <div
          ref={ref}
          onPointerMove={onPointerMove}
          className="spotlight lit-edge flex h-full flex-col overflow-hidden rounded-[12px] sm:rounded-[14px] border border-border/70 bg-surface shadow-2xs transition-all duration-300 hover:shadow-md hover:border-canopy/30 premium-card-container"
        >
          {/* Image & Overlay Badges Header */}
          <div data-tone="dark" className="relative aspect-[1.25/1] w-full overflow-hidden premium-card-img-container">
            <Image
              src={pkg.image}
              alt={pkg.title}
              fill
          ...
          {/* Card Details Body */}
          <div className="relative z-[3] flex flex-1 flex-col justify-between p-2.5 sm:p-3 pt-0.5 sm:pt-1 premium-card-body">
            <div>
              {/* Title */}
              <h3 className="font-sans text-[12.5px] sm:text-[13.5px] font-bold leading-snug tracking-tight text-ink line-clamp-2 group-hover:text-canopy transition-colors premium-card-title">
                {pkg.title}
              </h3>

              {/* Location Row */}
              <div className="mt-1 flex items-center gap-1 text-[10px] sm:text-[11px] font-medium text-ink-muted premium-card-location">
                <MapPin className="h-2.5 w-2.5 sm:h-3 sm:w-3 text-canopy shrink-0" />
                <span className="truncate">{pkg.location}</span>
              </div>

              {/* Date & Batches Row */}
              <div className="mt-0.5 flex items-center justify-between gap-1 text-[10px] sm:text-[11px] font-medium text-ink-muted premium-card-meta">
            ...
            {/* Footer Row */}
            <div className="mt-2 flex items-center justify-between border-t border-border/60 pt-2 premium-card-footer">
              <div className="flex items-baseline gap-0.5 premium-card-price-container">
                ...
                <span className="text-[13.5px] sm:text-[14.5px] font-extrabold text-ink premium-card-price">
                  {formatPrice(pkg.price)}
                </span>
                <span className="text-[9.5px] sm:text-[10px] font-normal text-ink-muted premium-card-price-slash">
                  /person
                </span>
              </div>

              <div className="flex items-center gap-0.5 text-[10px] sm:text-[11px] font-bold text-ink premium-card-rating">
